...
Logs activity related to extension background operations.

### `cron.log_statement`

<Admonition type="caution">

Can produce high levels of log activity for servers with frequent cron executions.
...
If you're using the CLI, you can add the `--no-restart` flag to postpone it. However, the new setting won't take effect until the database is restarted.
