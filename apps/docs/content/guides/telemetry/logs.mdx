...
Refer to the full field reference for each available source below. Read a listed path as a `log_attributes` key: the path `request.method` is `log_attributes['request.method']`.
