...
description: 'Supabase Pipelines is now in public alpha with schema change support, a faster initial copy, and new destination request forms for ClickHouse, Snowflake, and DuckLake.'
...
The pipeline is designed for reliable data movement. If a destination is temporarily unavailable or the pipeline restarts, replication resumes from the last acknowledged position. It can also self-heal under most circumstances.
...
We're opening destination request forms for:

- ClickHouse
- Snowflake
- DuckLake

These forms help us prioritize the next destinations based on real use cases: scale, data volume, expected latency, destination setup, and how teams want to query replicated data.
...
- **$39 per month per active pipeline**
- **$0.60 per GB** for the initial table copy
- **$3 per GB** for replicated data after the initial copy

An active pipeline is a pipeline that is configured and running replication. Initial table copy usage is measured during the first load of existing data. Replicated data usage is measured from changes processed after the initial copy. Pricing is subject to change during the public alpha. We'll give notice before any change takes effect.
